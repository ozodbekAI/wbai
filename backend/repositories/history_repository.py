...
class HistoryRepository:
    ...
    def get_user_history(
        ...
        return query.order_by(
            desc(ProcessingHistory.created_at)
    ...
    def get_statistics(
        ...
        records = self.db.query(ProcessingHistory).filter(
            ProcessingHistory.user_id == user_id,
        ...
        avg_time = sum(r.processing_time for r in records if r.processing_time) / total if total > 0 else 0
        avg_score = sum(r.validation_score for r in records if r.validation_score) / completed if completed > 0 else 0
